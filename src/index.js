...
function Settings() {
  ...
  return React.createElement(
    ...
    React.createElement(FormInput, {
      label: "Google Gemini API Key", value: geminiKey, placeholder: "AIzaSy...",
      onChange: (val) => { setGeminiKey(val); storage.geminiKey = val; }
    }),
    React.createElement(FormInput, {
      label: "OpenAI API Key (Optional)", value: openAiKey, placeholder: "sk-...",
      onChange: (val) => { setOpenAiKey(val); storage.openAiKey = val; }
    }),
    React.createElement(FormInput, {
      label: "Active Provider (gemini or openai)", value: provider, placeholder: "gemini",
      onChange: (val) => { const p = (val || "gemini").toLowerCase(); setProvider(p); storage.provider = p; }
    })
  );
}
    ...
    if (typeof registerCommand === "function") {
      unregister = registerCommand({
        ...
        options: [
          ...
          { name: "action", description: "Mode (ask, summarize, translate)", type: 3, required: false,
        ...
        execute: (args) => {
          ...
          return askAI(textArg, actionArg)
            .then(result => ({ content: "**[AI " + actionArg.toUpperCase() + "]:**\n" + result }))
